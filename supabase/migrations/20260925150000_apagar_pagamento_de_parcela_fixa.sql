-- Apagar o pagamento de parcela fixa depois de editar o contrato (25/09/2026).
--
-- Visto em produção: financiamento de 12 × R$ 831,48, "Paguei" às 11:24 e o contrato editado às
-- 11:28. A edição recalcula o saldo pelo contrato (parcela × restantes), e o pagamento guardou o
-- saldo de antes. Apagar confere os dois e recusava com "Corrija primeiro o pagamento mais
-- recente; o saldo mudou após este pagamento" — no ÚNICO pagamento da dívida. No app o toast dizia
-- só "Não deu para apagar. Tenta de novo.", e tentar de novo falhava para sempre.
--
-- Em parcela fixa a conferência não protege nada: o `check` do contrato já amarra o saldo às
-- pagas. Apagar o mais recente (pela data, e no mesmo dia pelo nº) desfaz UMA parcela; apagar um
-- mais antigo que ele continua recusado, agora dizendo qual apagar. Dívida com juros não muda: lá
-- o saldo depende da ordem e do valor, e a conferência continua. O resto do trigger é o de
-- `20260925140000`, byte a byte.
-- Teste: `supabase/tests/parcela_fixa_com_encargo.sql`, caso 8.
...
begin
  ...
  -- Parcela fixa: apagar o pagamento MAIS RECENTE devolve UMA parcela. O saldo é o do contrato
  -- (parcela × restantes, o `check`), não o histórico gravado no pagamento: editado o contrato
  -- depois de pagar, esse histórico deixa de bater com o saldo, e a conferência abaixo recusava
  -- justamente o mais recente com "Corrija primeiro o pagamento mais recente".
  if tg_op = 'DELETE' and d.calculation_mode = 'fixed_installments' then
    if exists (select 1 from public.transactions t where t.debt_id = debt and t.id <> old.id
                 and (t.occurred_at > old.occurred_at
                   or (t.occurred_at = old.occurred_at and t.debt_payment_no > old.debt_payment_no))) then
      raise exception 'Apague primeiro o pagamento mais recente desta dívida';
    end if;
    -- Com as pagas já zeradas à mão no contrato, a parcela não conta mais: só a linha sai.
