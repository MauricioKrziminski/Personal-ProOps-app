-- O alerta de orçamento passa a contar o COMPROMETIDO.
--
-- `budgets_status_for` separou "já aconteceu" de "ainda pode não acontecer" na `20260909180000`.
-- Sem acompanhar aqui, o alerta olharia só o gasto e ficaria calado com um limite já tomado por
-- um boleto agendado — e o momento de avisar é enquanto o boleto NÃO saiu, que é quando ainda dá
-- para remanejar. É o mesmo raciocínio do destaque amarelo do YNAB: ele acende pelo que vem, não
-- pelo que passou.
--
-- O texto separa as duas metades, senão o número do alerta não bate com o da tela.
--
-- ⚠️ Corpo tirado de `pg_get_functiondef`, NÃO de uma migration antiga — a assinatura desta
-- função já mudou duas vezes (`alerts_push_enabled`, `alerts_whatsapp_enabled`) e copiar do
-- arquivo velho custou um 42P13 hoje mesmo.
