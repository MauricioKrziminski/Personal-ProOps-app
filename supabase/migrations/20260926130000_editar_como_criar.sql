...
--
-- 2b. Parcelar um lançamento que JÁ existe (`convert_transaction_to_installments`) também pergunta
--    as parcelas já pagas — era só na criação da compra.
...
-- ── 2b. parcelar um lançamento que já existe, com as já pagas ────────────────────────────────
drop function if exists public.convert_transaction_to_installments(uuid, bigint, integer, date, text, text, text, uuid);
...
begin
  ...
  -- "Parcelas já pagas", como na criação (`20260926130000`): as primeiras N ficam pagas, e a
  -- fatura vencida que só tem parcelas pagas desta compra é quitada junto.
  if p_paid_installments is not null and p_paid_installments > 0 then
    update public.transactions t set status = 'cleared'
     where t.installment_plan_id = plano and t.workspace_id = tx.workspace_id
       and t.installment_no <= p_paid_installments and t.status = 'pending';
    perform private.quitar_faturas_do_plano(plano);
  end if;
