...
import { balanceMessage, type AccountBalanceRow } from "../_shared/balance-text.ts";
...
async function executeAction(
  ...
  switch (action.type) {
    ...
    case "query_balance": {
      ...
      if (error) throw error;
      // A formatação vive em `_shared/balance-text.ts` para poder ser TESTADA — inline aqui ela
      // exigia Supabase de pé para rodar, e foi assim que três defeitos de dinheiro conviveram
      // meses no arquivo. A mesma aritmética da tela Contas e do agente Python.
      return balanceMessage((data ?? []) as AccountBalanceRow[], centsToBRL);
    }
