...
export default function GoalsScreen() {
  ...
  const startEdit = (goal: Goal) => {
    ...
    setEditing(goal);
    setName(goal.name);
    setTargetCents(goal.target_cents);
  };
      ...
      <SafeAreaView style={styles.safeArea}>
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          {(goals ?? []).map((goal, index) => (
            <GoalCard key={goal.id} goal={goal} index={index} onEdit={startEdit} />
          ))}
          ...
          {showForm ? (
            <GlassCard style={styles.form}>
              <ThemedText type="smallBold">
                {editing ? `Editando “${editing.name}”` : 'Nova meta'}
              </ThemedText>
              <TextInput
                value={name}
              ...
              <Pressable
                ...
                <ThemedText type="smallBold" style={styles.buttonLabel}>
                  {save.isPending ? 'Salvando…' : editing ? 'Salvar' : 'Criar meta'}
                </ThemedText>
              </Pressable>
          ...
          {(goals ?? []).length > 0 && (
            <ThemedText type="small" themeColor="textSecondary" style={styles.centered}>
              Toque numa meta para editar. Segure para arquivar.
            </ThemedText>
          )}
        </ScrollView>
      </SafeAreaView>
