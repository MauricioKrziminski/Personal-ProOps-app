...
function familia(type: string): "criou" | "consultou" | "erro" {
  ...
  if (type.startsWith("query_") || type === "simulate_purchase")
    return "consultou";
  return "criou";
}
...
function resumoSemana(eventos: AiEvent[]) {
  ...
  const recentes = eventos.filter(
    (e) => new Date(e.created_at).getTime() >= corte,
  );
  return {
    total: recentes.length,
    criaram: recentes.filter((e) =>
      e.actions.some((a) => familia(a.type) === "criou"),
    ).length,
    falharam: recentes.filter(
      (e) => Boolean(e.error) || e.actions.some((a) => a.type === "unknown"),
    ).length,
  };
}
...
export default function AiActivityScreen() {
    ...
    confirmDestructive(
      ...
      () =>
        undo.mutate(ids, {
          ...
          onError: () =>
            toast({ message: "Não deu para desfazer.", tone: "error" }),
        }),
  ...
  const linha = (event: AiEvent, index: number) => {
    ...
    const podeDesfazer =
      !desfeito && (event.created_transaction_ids ?? []).length > 0;
        ...
        {aberto ? (
          <View style={styles.detalhe}>
            ...
            <ThemedText type="small" themeColor="textSecondary" style={tabular}>
              {event.model} ·{" "}
              {(event.input_tokens ?? 0) + (event.output_tokens ?? 0)} tokens
  ...
  return (
    <Screen grouped onRefresh={refetch} refreshing={isRefetching}>
      <Stack.Screen
        options={{ title: "Atividade da IA", headerLargeTitle: true }}
      />
      ...
      {!isLoading && !isError && lista.length > 0 ? (
        <ThemedText
          ...
          style={styles.rodape}
        >
          Também dá para corrigir conversando: “muda o último pra 54”. Nota,
          lembrete e meta ainda não guardam o id de origem, então não têm
          desfazer aqui.
        </ThemedText>
      ) : null}
    </Screen>
